...
import frc.robot.commands.IntakeCmds.SetIntakeDownCmd;
import frc.robot.commands.IntakeCmds.SetIntakeUpCmd;
import frc.robot.commands.IntakeCmds.ToggleIntakeCmd;
...
public class RobotContainer {
        ...
        private final NoteDetectorSubsytem noteDetector = NoteDetectorSubsytem.getInstance();
        ...
        private void configureBindings() {
                ...
                OperatorB
                                .whileTrue(new ParallelCommandGroup(new RunBeltCmd(conveyor, -.9),
                                                new RunIntakeCmd(intake, -.9)));

                OperatorB
                                .whileTrue(new SetIntakeDownCmd()
                                                .andThen(new ParallelCommandGroup(new RunBeltCmd(conveyor, -.9),
                                                                new RunIntakeCmd(intake, -.9)))
                                                .onlyWhile(() -> !noteDetector.getRingStatus()))
                                .whileFalse(new SetIntakeUpCmd().andThen(new RunLauncherCmd(launcher, () -> 1000)));
        ...
        public void configureAutonCommands() {
                ...
                NamedCommands.registerCommand("longRangeIntakeCmd",
                                new RunIntakeCmd(intake, -.9375).withTimeout(1.725));
                ...
                NamedCommands.registerCommand("runLauncherCmd",
                                new RunLauncherCmd(launcher, () -> 1000).withTimeout(.75));
                ...
                NamedCommands.registerCommand("AutoAngleLauncher",
                                new LauncherAngleCmd(launcher,
                                                () -> -3.06 * drivebase.triangulateDistanceToSpeaker() + 10.2, false)
                                                .withTimeout(.75));
                NamedCommands.registerCommand("resetLauncher",
                                new LauncherAngleCmd(launcher, () -> 1.5, true).withTimeout(.5));
                NamedCommands.registerCommand("fixedDown",
                                new LauncherAngleCmd(launcher, () -> 2, true).withTimeout(.5));
        }
